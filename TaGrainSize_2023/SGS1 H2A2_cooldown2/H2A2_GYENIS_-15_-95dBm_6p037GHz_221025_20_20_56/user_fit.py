...
def power_sweep_fit_drv(atten=[0, -60], sample_name=None,
                        ...
                        plot_twinx=True, plot_fit=False, crop_pts=0):
    """
    ...
    filter_points = [[crop_pts, crop_pts] for _ in filenames]
    fpts = [[400, 400]]
...
if __name__ == '__main__':
    ...
    samp_name = 'H2A2_GYENIS'
    freq_str = '6p037GHz'
    dstr = '221025'
    temperature = 9e-3
    powers_in = np.linspace(-19, -95, 39)
    powers_in = np.linspace(-15, -95, 41)

    print(powers_in)

    # Does not change from sample-to-sample
    tstr = f'{temperature*1e3:.0f}mK'
    filenames_in = [f'{samp_name}_{dstr}_{freq_str[0]}_{freq_str}_{int(p)}dB_{tstr}.csv' 
                for p in powers_in]

    # Run the power sweep
    power_sweep_fit_drv(sample_name=samp_name+f'_{freq_str}',
                        ...
                        plot_twinx=False, plot_fit=True, crop_pts=0)
