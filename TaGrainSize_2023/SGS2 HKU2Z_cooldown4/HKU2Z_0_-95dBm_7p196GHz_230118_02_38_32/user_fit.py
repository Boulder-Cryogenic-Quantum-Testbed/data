...
if __name__ == '__main__':
    ...
    powers_hi = np.linspace(0, -35, 8)
    # powers_lo =  np.linspace(-40, -95, 12)
    powers_lo =  np.linspace(-40, -95, 12)
    powers_in = np.hstack((powers_hi, powers_lo, [-97, -99, -100, -103]))
    # Crop out high power points with strong quasiparticle response
    powers_in = powers_in[4:]
    print(powers_in)
    tstr = f'{temperature*1e3:.0f}mK'
    samp_name = 'HKU2Z'
    fcs = [4.49859, 4.90673, 5.29152, 6.07354,
           6.46026, 6.59051, 6.83411, 7.19602]
    dstr = '230118'
    fc = fcs[7]
    fstr = f'{np.floor(fc):.0f}_{fc:.3f}GHz'.replace('.', 'p')
    
    # Set the filename strings
    filenames_in = [f'{samp_name}_{dstr}_{fstr}_{int(p)}dB_{tstr}.csv' 
                for p in powers_in]
    # filenames_in = [f'{samp_name}_220930_6_6p112GHz_{int(p)}dB_{tstr}.csv' 
    #             for p in powers_in]

    # Run the power sweep
    power_sweep_fit_drv(sample_name=samp_name+f'_{fc:.3f}Hz'.replace('.', 'p'),
